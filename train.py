...
def train(generator_real, opt):
    ...
    if opt.game == 'mario':
        scaled_list = special_mario_downsampling(opt.num_scales, scales, generator_real, opt.token_list)
    else:  # if opt.game == 'mariokart':
        scaled_list = special_mariokart_downsampling(opt.num_scales, scales, generator_real, opt.token_list)

    generator_reals = [*scaled_list, generator_real]
    ...
    wandb.log({"generator_real": wandb.Image(img)}, commit=False)
    ...
    for current_scale in range(0, stop_scale):
        ...
        z_opt, input_from_prev_scale, G = train_single_scale(D1, D2,  G, generator_reals, generators, noise_maps,
