...
def get_all_tokens(opt, replace_tokens=REPLACE_TOKENS):
    ...
    # Get D1 tokens
    d1_txt_level = load_level_from_text("%s/%s" % (opt.input_dir, opt.discriminator1_input_name), replace_tokens)
    ...
    # Get D2 tokens
    d2_txt_level = load_level_from_text("%s/%s" % (opt.input_dir, opt.discriminator2_input_name), replace_tokens)
